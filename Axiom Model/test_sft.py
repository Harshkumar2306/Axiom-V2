...
def load_sft_model(ckpt_path, device):
    ...
    print(f"✅ SFT Brain loaded successfully! (Step: {step}, Best Val Loss: {val_loss})")
    return model

def ask_axiom(model, enc, user_query, device, max_new_tokens=250, temperature=0.7, top_p=0.9, rep_penalty=1.15):
    prompt = (
        "### System:\n"
...
def main():
    ...
    # Priority: checkpoints_sft/best.pt -> checkpoints_sft/latest.pt
    ckpt = "checkpoints_sft/best.pt"
    if not os.path.exists(ckpt):
        ckpt = "checkpoints_sft/latest.pt"
    if not os.path.exists(ckpt):
        print(f"❌ Checkpoint not found at {ckpt}!")
        return
        
    model = load_sft_model(ckpt, device)
    
    test_queries = [
        "Who are you and what are your capabilities?",
        "What are the four nitrogenous bases found in DNA?",
        "Explain the law of conservation of energy in simple terms.",
        "Write a Python function to check if a number is prime.",
    ]
    
    print("\n🚀 Commencing Axiom V2 Phase 4 Litmus Tests...\n")
    for q in test_queries:
        ask_axiom(model, enc, q, device)
        
    print("\n" + "="*70)
    print("🎉 SFT Verification Complete!")
    print("="*70 + "\n")
